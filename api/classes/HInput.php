...
final class HInput {
    ...
    public static function format_currency($amount, $currencyCode) {
        // Create a NumberFormatter for currency formatting
    ...
    public static function rateLimitFile($max_requests, $period_in_seconds) {
    ...
    public static function rateLimit($max_requests, $period_in_seconds) {
        ...
        // Check if session is set for rate limiting
        ...
        // Create a unique key for the session based on the URL
        $session_key = md5($request_uri);
    
        // Initialize or retrieve rate limit data for this session key
        if (!isset($_SESSION['rate_limit'][$session_key])) {
            $_SESSION['rate_limit'][$session_key] = [
                'timestamp' => time(),
                'count' => 1
            ];
        } else {
            $data = &$_SESSION['rate_limit'][$session_key];
    
            // Check if the current time is within the rate limit period
            if (time() - $data['timestamp'] < $period_in_seconds) {
